...
// update table users for points count for the commenter
...
curl_setopt($ch, CURLOPT_POST, TRUE);
curl_setopt($ch, CURLOPT_HTTPHEADER, array("Authorization: Bearer $tg_accesstoken"));  
curl_setopt($ch, CURLOPT_POSTFIELDS, $data_param3);
//curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
$output = curl_exec($ch); 
...
$res = $json["results"][0]["accepted_vertices"];
if($res == 1){
//echo "<div style='background:green;color:white;padding:8px;border:none;'>Data Successfully Updated 3</div>";
}



// update table users for points count for the poster
